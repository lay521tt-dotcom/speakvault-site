...
- Included: listening library, search/scenario/difficulty/progress filters, dictation notes, dated practice history, manual checkpoints, record safety self-check, hidden subtitles, expression mining, phrase vault, local practice records, JSON backup/restore, copyable backup text, AI writing feedback.
- Data storage: browser `localStorage` for each user/device, written to a primary key and local mirror key, with manual JSON export/import, non-destructive import merge, and copyable backup text. Export regularly before clearing browser data or changing devices.
