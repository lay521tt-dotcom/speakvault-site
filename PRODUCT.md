...
- visible autosave and last-backup status for local record safety;
...
V1 does not include login, database sync, uploads, speech scoring, pronunciation correction, realtime voice, user management, or content licensing workflow. Records are safe for daily use on the same browser when the user exports JSON backups regularly; cross-device automatic persistence requires a future cloud-sync version.
